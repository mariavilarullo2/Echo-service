...
final http:Client backendClient = check new("http://localhost:9092");

service /call on new http:Listener(9090) {

    isolated resource function get all() returns json|error {
    ...
    isolated resource function get '5xx() returns json {
       ...
       isolated resource function get '4xx() returns json {
